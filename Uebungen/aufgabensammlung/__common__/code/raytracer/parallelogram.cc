...
namespace rtai {
  ...
  HitInfo Parallelogram::isHitBy( const Ray &ray )
  {
    HitInfo hi;
    ...
    //   Strahl: x = o + l*d
    // und
    //   Parallelogramm: x = p + l1*d1 + l2*d2
    //                   0 <= l1,l2 <= 1
    // gilt:
    ...
    if (params(0) == 0.0) {
      // Strahl liegt in Parallelogramm, wenn Ausgangspunkt in Parallelogramm
      // liegt
      ...
      // Strahl trifft Ebene, in der das Parallelogramm liegt
